...
| `MAPS` | Campaign hex maps: `#maps` mode, Pixi editor (`maps/maps-pixi.bundle.js` + `maps/maps-app.js`), IndexedDB `bg-maps`, bundled `maps/starter.hexplora`, HexPlora import/export, optional Library `ref` id on a token (`vRef` in `vToken`; Open / Tracker / Board chips from `actionsFor`); token icons are picker slugs with PUA codepoints or foreign Material Symbols names kept by `normalizeTokenIcon` and drawn by ligature (`tokenIconSprite` caches textures, never sprites); `exportRecords` / `importRecords` / `removeRecords` record bridge for the campaign archive, plus transactional `replaceRecords` (flush → snapshot → write → prefix-exact undo) as the archive's only write path |

...
- Every control in the dialog — the three chips plus **Cancel import** and **Close** — goes `aria-busy` + disabled while a download builds or an import reads/applies: neither can be stopped once running, and a Cancel that dismissed the row while the apply committed anyway would be a button that lies. The backdrop click still dismisses the overlay, so a hung build never traps anyone

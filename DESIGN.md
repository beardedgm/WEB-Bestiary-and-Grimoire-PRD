...
- **Maps:** Map-first `#maps` shell — **Maps** / **Settings** toolbar chips open mutually exclusive drawer trays (`maps` | `settings` | `tool` | `danger` views on `#mapsDrawer`); tool chips auto-open the tool tray. The map list is a `div.maps-list-row` per map — title button (`.maps-list-item`, `.on` + `aria-current` on the open map) | **Rename** | **✕** — so rename and delete work on any row, open or not: Rename swaps the title for an inline input (Enter/blur commit, Escape cancel, 80 chars, empty → "Map"); ✕ confirms with `TRK.confirmSwap`, and the confirm takes the whole row over while it is live (`:has(.cfm)` hides the title and Rename) so the label names the map instead of squeezing beside it. Neither lives in Settings. **Reset view**, **PNG** and **Export** (the open map as `.hexplora`) are toolbar chips after the shape cluster behind a hairline `.maps-tool-sep` (hidden at ≤760px, where the toolbar wraps) — actions, never `aria-pressed`; both toast "Open a map first" with nothing open. Toolbar spans full width **above** the drawer/canvas row so neither the app header nor the maps tool bar is covered. Desktop: drawer is an in-flow flex sibling that **pushes** the canvas aside (260px / collapsed); width **snaps** in both directions, so opening eases `opacity`/`transform` (280ms) while closing is instant — the zero-width clip lands first, so there is nothing left to fade. Mobile: bottom sheet + scrim scoped to `.maps-body` only (`transform: translateY`). A closed drawer is `inert` in every state, so its off-screen controls leave the tab order (`pointer-events:none` blocks the mouse but not Enter); closing while focus is inside hands focus back to the visible toggle, since `inert` does not blur what is already focused. Pixi stage fills remaining space. Tokens: disc + centered SVG icon (`maps/token-icons/`) + label below (HexPlora layout). Edit overlays sit outside `<main>`. At **≤760px**, **Maps** FAB + scrim.
...
- Touch/coarse pointer: 44px minimum on tracker damage/heal/remove/reveal/drag, catalog add, dice tray controls, Board card ops / primary card controls, Forge band buttons, Lore editor / format / tree-add chips, and Maps list-row controls (title / Rename / ✕) on the narrow breakpoint. The shared rules sit in one `coarse-pointer targets` block at the end of the stylesheet so they follow every component rule they enlarge; a rule for the same selector placed earlier loses on source order

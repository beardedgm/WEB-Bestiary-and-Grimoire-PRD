...
- Tracker mode: initiative order, rounds, drag-reorder, undo/redo, party library, encounter presets, dice tray, player display (a stone board of equal paper rows with the active row filled in ink as the only turn cue; monsters masked as **Enemy N** until the GM presses the card's reveal chip (it reads **Enemy N**, then **Revealed**), players and NPCs by name; markers and Hurt / Bloody / Unconscious badges, never HP; a static roll card; nothing animates); a **glance strip** above the combatant list names who acts now and who is next and lists the last few things that happened (damage, healing, turns, adds, markers) — in-memory only, ~14 entries, gone on reload, never persisted or exported and never a rules engine
...
- Ad-hoc colored markers on combatants (five colors, solid/outline) mirrored on the player display — table-ring shorthand, no condition legend; white takes an ink edge on the display's light rows
- Catalog vs Table navigation in the left column (monsters/spells vs party/encounters)
- Custom library: content-only authoring JSON in `+` (5e/PF2e Fireball and Lich stubs; plumbing omitted); Import stamps id/schema/source from the chosen system and deep-validates; Custom filter; every record's reading pane carries a **JSON** disclosure showing its stored record with **Copy** / **Download** (corpus read-only); a custom's **Edit** reopens the same dialog seeded with its authoring JSON and **Save** keeps the id stable through renames, so Board cards, map token refs, rosters and presets keep resolving; **Edit a copy** forks any record — corpus or custom — into a new custom whose id comes from the new name; remove from library; Forge Save to Custom uses the same store
...
- Player display is supplementary; core GM workflow must remain keyboard-accessible. The display is static by construction (no transitions, animations or promoted layers), because it is cast to a TV from a Chrome tab and those are what freeze the cast.

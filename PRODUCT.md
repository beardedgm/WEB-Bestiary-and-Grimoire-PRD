...
- Maps is the only mode that loads PixiJS (`maps/maps-pixi.bundle.js`); rebuild with `npm run build:pixi` in `maps/`
- Render free-tier hosting constraints apply if deployed to Render (ephemeral FS, bind `0.0.0.0:$PORT` for any future server)

**Terminology:** Catalog, Table, Library, Tracker, Board, Builder, Forge, Lore (durable campaign pages), Campaign (owns party / presets / lore / maps meta), Maps, party, encounter preset, threat/difficulty budget, stat block, spine (system color mark on list rows), custom library, user save, counter, random table.

...
Bestiary & Grimoire is a browser-based reference, campaign notebook, and combat assistant built on a normalized 9,339-record corpus (monsters and spells for both game systems). It lets a GM search and read stat blocks, maintain campaign Lore, add creatures to an encounter, run initiative, track HP, roll dice from stat-block formulas, manage a party roster and saved encounters, and mirror initiative on a player display — all without leaving the browser or sending data off-machine.
...
- **Play:** Switch to Tracker; add monsters from Catalog or party from Table; roll initiative, advance turns, apply damage/healing; click formulas in stat blocks to roll; optional player display window (1920×1080). Switch to Builder to budget a PF2e / 5e 2014 / 5e 2024 encounter from party size + level, then Save to Encounters or Load into Tracker. Switch to Board for spatial session notes, maps, timers, and tools. Switch to Forge to build a custom 5e or PF2e creature from CR/level benchmarks and Save to Custom. Switch to Lore for durable campaign pages organized by campaign, page tree, and tags; pin a whole page to tonight's Board without leaving Lore.
- **Persistence:** Encounter state, party, saved encounters, dice history, column widths, UI mode, Builder draft, Board sessions, and Lore campaigns persist in `localStorage` (storage failures are announced; portable import reports failure when a write fails). Custom monsters/spells live under `bg.custom.records.v1` (validated on load; invalid rows dropped with notice). Boards live under `bg.board.v1` (cards validated on load/import; runtime preview cache fields are not persisted). Lore lives under `bg.lore.v1` (campaigns and pages validated on load/import). Builder draft under `bg.builder.v1`. Encounter presets may carry optional `builder` metadata for round-trip reopen. A portable `bg-user-save/1` JSON export merges customs/party/presets/boards/lore by id (local-only kept) and replaces the active encounter after confirmation; import rejects files over **8 MB**, and export warns when the download would exceed that cap.
...
- Board mode: multi-session spatial boards (`bg.board.v1`) with markdown (incl. GFM tables / read-aloud blockquotes), image, audio clip, counter, dice, timer (countdown/stopwatch), checklist, random table, and linked record cards (a Library monster/spell referenced by id, rendered live — "Send to Board" from the reading pane; a removed custom shows a missing-record notice, never a stale copy) and linked encounter cards (a saved Encounter preset referenced by id — roster summary, Load into Tracker / Open in Builder, combatant names spawn record cards; "Send to Board" from Table and from Builder when the draft is saved); snap-grid drag/resize; portable save includes boards
...
- Lore mode: multi-campaign markdown pages (`bg.lore.v1`) with nested page tree (per-row **Add child**, drag-handle reorder/reparent, Alt+Arrow sibling move and indent/outdent; drag disabled while filtered), tags and filtering, edit/preview with a grouped format chip toolbar for Board-compatible markdown (bold / italic / inline code; H1–H3; bullet and numbered lists; read-aloud quotes; fenced blocks; tables), visible Pin/save toasts, empty-state guidance for the Pin-to-Board loop, and **Pin to Board** as the primary editor action; linked Lore cards render the current page markdown and show an honest missing-page notice after deletion; portable save includes Lore campaigns/pages; on narrow viewports the campaign rail opens as a bottom sheet
...
**Terminology:** Catalog, Table, Library, Tracker, Board, Builder, Forge, Lore (durable campaign pages), party, encounter preset, threat/difficulty budget, stat block, spine (system color mark on list rows), custom library, user save, counter, random table.
...
6. **Create once, use everywhere** — reusable game content lives in the Library keyed by id, and durable campaign truth lives in Lore keyed by campaign/page id; other surfaces reference those sources rather than copying them (see `docs/superpowers/specs/2026-08-26-connected-workflow.md`).
